...
if ~isempty(data)
    ...
    if isfield(data.exper,'odor_rlwm')
        CountedTrial=data.exper.odor_rlwm.param.countedtrial.value;
        Result=data.exper.odor_rlwm.param.result.value(1:CountedTrial);
        portside=data.exper.odor_rlwm.param.port_side.value(1:CountedTrial);
        schedule=data.exper.odor_rlwm.param.schedule.value(1:CountedTrial);
        OdorChannelSchedule=data.exper.odor_rlwm.param.odorchannel.value(1:CountedTrial);
        OdorName=data.exper.odor_rlwm.param.odorname.value(1:CountedTrial);
...
%         boxrig=data.exper.control.param.expid.value;
        protocol='odor_rlwm';
        StimParam=data.exper.odor_rlwm.param.stimparam.value;
        param_string=data.exper.odor_rlwm.param.stimparam.user;
    ...
    valid_trials=size(Result).*0;
    for k=1:CountedTrial
        if k==1
            tt1=0;
            try
                if ismember(Result(k),[1.2, 1.3]) % two or three drop H2O
                    tt2=data.exper.odor_rlwm.param.trial_events.trial{k}(end,3);
                    kk=kk+1;
                else
                    tt2=data.exper.odor_rlwm.param.trial_events.trial{k}(:,3);
                    if length(tt2)>1
                        tt2=tt2(1);
                    end
                    kk=kk+1;
            ...
            tt1=tt2;
            if ~isempty(data.exper.odor_rlwm.param.trial_events.trial{k})
                if ismember(Result(k),[1.2, 1.3]) % two or three drop H2O
                    tt2=data.exper.odor_rlwm.param.trial_events.trial{k}(end,3);
                    kk=kk+1;
                else
...
%                     end
        ...
        % OdorOn Time
        if data.exper.odor_rlwm.param.delayodor.value==1
