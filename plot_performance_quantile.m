...
figure;
sgtitle(['Performance in the first 2 ', tlabel, ' sessions in quantile'])
subplot(2,1,1)

genotype_list = unique(genotype);

for gg = 1:length(genotype_list)
    mean_perf = nanmean(data(strcmp(genotype, genotype_list{gg}),:,1),1);
    if sum(strcmp(genotype, genotype_list{gg}))>1
        ste_perf = nanstd(data(strcmp(genotype, genotype_list{gg}),:,1),1)/sum(strcmp(genotype, genotype_list{gg}));
    else
        ste_perf = [0, 0, 0, 0];
    end
    %plot(mean_perf)
...
for gg = 1:length(genotype_list)
    mean_perf = nanmean(data(strcmp(genotype, genotype_list{gg}),:,2),1);
    if sum(strcmp(genotype, genotype_list{gg}))>1
        ste_perf = nanstd(data(strcmp(genotype, genotype_list{gg}),:,2),1)/sum(strcmp(genotype, genotype_list{gg}));
    else
        ste_perf = [0, 0, 0, 0];
    end    %plot(mean_perf)
    hold on;
