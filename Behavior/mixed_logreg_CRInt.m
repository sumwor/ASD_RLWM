function mixed_logreg_CRInt(dataIndex,tlabel, nSession, step_back, savefigpath, savedatapath)
...
sgtitle(['mixed CRInt ', tlabel,num2str(nSession)]);
...
print(gcf,'-dpng',fullfile(savefigpath, ['mixed_CRInt_', tlabel,num2str(nSession)]));    %png format
saveas(gcf, fullfile(savefigpath, ['mixed_CRInt_', tlabel,num2str(nSession)]), 'fig');
saveas(gcf, fullfile(savefigpath, ['mixed_CRInt_', tlabel,num2str(nSession)]),'svg');

% save the logstic regression result
savelogname = fullfile(savedatapath, ['mixed_CRInt_', tlabel,num2str(nSession),'.mat']);
