function mixed_logreg_RCUC(dataIndex,tlabel, nSession, step_back, savefigpath, savedatapath)
...
sgtitle(['mixed RCUC ', tlabel,num2str(nSession)]);
...
print(gcf,'-dpng',fullfile(savefigpath, ['mixed_RCUC_', tlabel,num2str(nSession)]));    %png format
saveas(gcf, fullfile(savefigpath, ['mixed_RCUC_', tlabel,num2str(nSession)]), 'fig');
saveas(gcf, fullfile(savefigpath, ['mixed_RCUC_', tlabel,num2str(nSession)]),'svg');

% save the logstic regression result
savelogname = fullfile(savedatapath, ['mixed_RCUC_', tlabel,num2str(nSession),'.mat']);
